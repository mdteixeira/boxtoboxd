...
import { fetchBooksRatings, fetchMatchesRatings } from '../service/FirebaseServices';
import AddRating from '../components/RatingPopup';
import CardAvaliacao from '../components/CardAvaliacaoPartidas';
import CardAvaliacaoLivro from '../components/CardAvaliacaoLivro';

function ListRatings() {
    const [ratings, setRatings] = useState<any>([]);
    const [bookratings, setBookRatings] = useState<any>([]);
    ...
    useEffect(() => {
        fetchMatchesRatings(setRatings);
        fetchBooksRatings(setBookRatings);
    }, []);
        ...
        <>
            <div className="w-full container mx-auto sm:flex justify-between py-2 items-center dark:bg-slate-900 bg-white bg-opacity-75 rounded-3xl px-2 ps-6 mt-2 hidden">
                {ratings.length == 1 ? (
                    ...
                    <h3>{ratings.length} avaliações de partidas</h3>
                )}
                <div className="flex gap-4">
                    ...
                    <button
                        onClick={() => fetchMatchesRatings(setRatings)}
                ...
                <div
                    ...
                    {ratings?.map((rating: any) => (
                        <CardAvaliacao
                            ...
                            comentario={rating.comentario}></CardAvaliacao>
                    ))}
                </div>
            ) : (
                <div className=" container mx-auto flex flex-col justify-center h-[75vh]">
                    <SoccerBall
                        className="text-emerald-700 h-1/3 w-full"
                        weight="duotone"
                    />
            ...
            <div className="w-full container mx-auto sm:flex justify-between py-2 items-center dark:bg-slate-900 bg-white bg-opacity-75 rounded-3xl px-2 ps-6 mt-2 hidden">
                {bookratings.length == 1 ? (
                    <h3>1 avaliação de livros</h3>
                ) : (
                    <h3>{bookratings.length} avaliações de livros</h3>
                )}
                <div className="flex gap-4">
                    <div className="inline-flex items-center gap-3 relative">
                        <input
                    ...
                    <button
                        onClick={() => fetchBooksRatings(setBookRatings)}
                        className="px-3 p-2 bg-opacity-5 bg-emerald-500 border border-emerald-500 rounded-2xl font-medium inline-flex items-center gap-2 hover:bg-opacity-100 text-emerald-500 hover:text-white">
                        <ArrowCounterClockwise weight="bold" className="text-xl" />
                        <p className="text-sm">Reload</p>
                    </button>
